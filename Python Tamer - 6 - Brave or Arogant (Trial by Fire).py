...
while True:
    ...
    if pilih == "1":
        ...
        try:
            ...
            # Buka file dan pastikan for loop ada di DALAM blok 'with'
            with open('absensi.csv', 'r', newline='') as file:
                reader = csv.reader(file)
                ditemukan = False
                
                print("\n--- Hasil Pencarian ---")
                for baris in reader:
                    # Pastikan baris tidak kosong dan cek indeks ke-1 (Nama)
                    # Sesuai urutan simpan kita: [ID, Nama, Waktu_Masuk]
                    if len(baris) >= 3 and baris[1].title() == cari_nama:
                        print(f"Data Ditemukan -> ID Karyawan: {baris[0]} | Nama: {baris[1]}")
                        ditemukan = True
                        # Opsional: Jangan gunakan 'break' di sini jika 
                        # ada kemungkinan nama karyawan kembar (misal ada 2 "Janan")
                
                if not ditemukan:
                    print(f"Data atas nama '{cari_nama}' tidak ditemukan.")
                    
        except FileNotFoundError:
            print("Belum ada data absensi untuk dicari.")
    elif pilih == "3":
